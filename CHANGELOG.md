...
- **tsconfig:** `noUncheckedIndexedAccess` enabled; the 11 `scripts/` sites it flagged were `NAMED_COLORS` index-signature lookups, moved onto `Color.*` constants or the string-color path. Emitted `.d.ts` files are byte-identical — no public API or runtime change (#36).
...
- **color:** `parseHexString()`, `tryResolveColor()`, and `resolveColor()` now reject malformed hexadecimal strings instead of accepting partially valid pairs (#16).

## [0.7.5] — 2026-08-13

### Fixed

- **image:** `downsampleSprite()` now clamps quantized body-color channels to valid RGB bytes, so saturated image colors return 255 rather than 260 (#15).

## [0.7.4] — 2026-08-13

### Fixed

- **animation:** `pushAnimation()`, `encodeAnimationGif()`, and `saveAnimationGif()` now reject mixed-size frame sequences with `RangeError` before device requests, GIF encoding, or file writes (#18).

## [0.7.3] — 2026-08-13

### Fixed

- **client:** `send(command, params)` now keeps the explicit command authoritative when `params` also contains a top-level `Command`, while preserving all other parameters (#19).
- **client:** HTTP retries now apply only to 408, 429, 500, 502, 503, and 504; other HTTP failures return immediately without repeating the command (#20).

## [0.7.2] — 2026-08-13

### Fixed

- **canvas:** `drawCircle()` now rejects `NaN` and infinite radii with `RangeError` before rasterization, preventing non-finite values from hanging the midpoint-circle loop (#23).
...
- `assets/social_preview_header.png` — social preview image for GitHub repository.

### Changed

- README badges consolidated to a single inline row.
