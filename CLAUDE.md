...
Keep `CLAUDE.md` and `AGENTS.md` byte-identical. Update both when adding scripts, source modules, or discovering new device API behaviors; the Scripts table and Project Structure section should reflect the current state of the repo.

Before release, run `bun run devcheck` and `bun run test:all`. `prepublishOnly` enforces both gates for npm publishes.
